...
class BonLivraisonController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Ajout des produits au Bon de Livraison
        foreach ($request->products as $product) {
            $total += $product['qte'] * $product['prixUnitaire'];
        ...
        // Ajout des paiements associés
        foreach ($request->paiements as $paiement) {
            Paiement::create([
                'idBonDeLivraison' => $bonLivraison->id,
                'mode' => $paiement['mode'],
                'montant' => $paiement['montant'],
                'echeanceAt' => $paiement['echeanceAt'] ?? null,
            ]);
        }

        // Mise à jour du total du Bon de Livraison
        $bonLivraison->update(['totale' => $total]);
    ...
    public function show(BonLivraison $bonLivraison)
    {
        return response()->json($bonLivraison->load('client', 'items', 'paiements'));
    }
